...
class WeatherDate: ObservableObject{
    ...
    func searchWeather(keyword: String) {
        ...
        //リクエストURLの組み立て
        guard let req_url = URL(string: "https://api.openweathermap.org/data/2.5/onecall?lat=35.698785&lon=139.696548&exclude=current,minutely,hourly,alerts&units=metric&"+"appid="+appid+"&lang=ja") else {
            print("return2")
            return
        }
